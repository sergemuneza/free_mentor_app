...
//     id: data["id"],
//     name: "${data["firstName"] ?? 'Unknown'} ${data["lastName"] ?? ''}".trim(), // ✅ Default name
//     bio: data["bio"] ?? "No bio available", // ✅ Default bio
//     occupation: data["occupation"] ?? "Unknown occupation", // ✅ Default occupation
//     expertise: data["expertise"] ?? "Unknown expertise", // ✅ Default expertise
//   )).toList();
// }
...
class DBHelper {
  ...
  //   return results.map((data) => User.fromJson(data)).toList();
  // }
  // ✅ Fetch all users (Only users with role = "user", Exclude Admins)
static Future<List<User>> getAllUsers() async {
  final db = await database;
  List<Map<String, dynamic>> results = await db.query(
    "users",
    where: "role = ?", // ✅ Fetch only normal users
    whereArgs: ["user"],
  );
